...
func appPostRideEvaluatation(w http.ResponseWriter, r *http.Request) {
	...
	setRideStatus(rideID, "COMPLETED")
...
func getRideStatus(rideID string) (string, bool) {
	if s, ok := rideStatusMap[rideID]; ok {
		return s, true
	}
	return "", false
}
...
func getChairStats(ctx context.Context, tx *sqlx.Tx, chairID string) (appGetNotificationResponseChairStats, error) {
	...
	err := tx.SelectContext(
		ctx,
		&rides,
		`SELECT rides.evaluation FROM rides INNER JOIN ride_statuses ON ride_statuses.ride_id = rides.id WHERE chair_id = ?`,
		chairID,
	)
	if err != nil {
		return stats, err
